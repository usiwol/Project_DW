...
const LikeCamping = () => {
  ...
  const getUserPick = () => {
    axios.get('/pick/' + localStorage.getItem('userId'), {}).then((response) => {
  ...
  const doClick = (campIdx) => {
    if (pick) {
      console.log('찜 취소');
      axios.delete('/pick/' + campIdx + '&' + localStorage.getItem('userId'), {}).then((response) => {
          ...
          {data.map((d, i) => {
            return (
              <div className="contentsLikeCamping" key={i}>
                <div className="contentsImgLikeCamping"></div>
                <div className="contentsTextLikeCamping">
                  <div className="contentsRowFlexLikeCamping">
                    <img onClick={() => doClick(d.campsite.campsiteIndex)} src={heart} alt="찜아이콘" className="contentsHeartLikeCamping"></img>
